...
function Start-OneDriveMigrationShareGate {
    ...
    Set-Variable srcSite, dstSite, srcList, dstList, srcSiteUrl, dstSiteUrl, dstSiteUrlInitial, dstSiteUrl
    $AllOneDriveErrors = @()
    $AllOneDriveResults = @()
    ...
    foreach ($user in $OneDriveUsers) {
        ...
        Clear-Variable srcSite, dstSite, srcList, dstList, srcSiteUrl, dstSiteUrlInitial, dstSiteUrl
        ...
        if ($srcList) {
            ...
            elseif ($dstSiteUrlInitial = Get-OneDriveUrl -Tenant $destinationTenant -Email $user.UserPrincipalName_Destination -ErrorAction SilentlyContinue) {
                if ($DomainTLD -eq "com") {
                ...
                #If Site does not exist check for UPN
