...
function Get-GroupDetails {
    ...
    foreach ($object in $allMailGroups) {
        ...
        elseif ($object.RecipientTypeDetails -eq "GroupMailbox") {
            ...
            $currentobject | add-member -type noteproperty -name "GroupOwners" -Value ($groupOwners.PrimarySmtpAddress.ToString() -join ",") -Force
            $currentobject | add-member -type noteproperty -name "MembersCount" -Value ($groupMembers | measure-object).count-Force
            $currentobject | add-member -type noteproperty -name "Members" -Value ($groupMembers.PrimarySmtpAddress.ToString() -join ",") -Force
