...
foreach ($user in $sourceMailboxes) {
    ...
    if ($user.ArchiveStatus -eq "Active") {
        $ArchiveStats = Get-EXOMailboxStatistics $user.primarysmtpaddress -Archive -ErrorAction silentlycontinue | select TotalItemSize, ItemCount     
...
# Gather Mailbox Stats - Hash table
$sourceMailboxes = Get-ExoMailbox -ResultSize Unlimited -Filter "PrimarySMTPAddress -notlike '*DiscoverySearchMailbox*'" -PropertySets archive, delivery, minimum
$sourceMailboxStats = @{}
$progressref = ($sourceMailboxes).count
$progresscounter = 0
foreach ($user in $sourceMailboxes) {
    ...
    $currentuser["IsLicensed"] = $msoluser.IsLicensed
    $currentuser["Licenses"] = ($msoluser.Licenses.AccountSkuID -join ";")
    $currentuser["License-DisabledArray"] = ($msoluser.LicenseAssignmentDetails.Assignments.DisabledServicePlans -join ";")
    $currentuser["Department"] = $msoluser.Department
    $currentuser["RecipientTypeDetails"] = $user.RecipientTypeDetails
    $currentuser["PrimarySmtpAddress"] = $user.PrimarySmtpAddress
    $currentuser["Alias"] = $user.alias
    $currentuser["WhenCreated"] = $user.WhenCreated
    $currentuser["LastLogonTime"] = $mbxStats.LastLogonTime
    $currentuser["LastInteractionTime"] = $mbxStats.LastInteractionTime
    $currentuser["EmailAddresses"] = ($EmailAddresses -join ";")
    $currentuser["LegacyExchangeDN"] = ("x500:" + $user.legacyexchangedn)
    $currentuser["HiddenFromAddressListsEnabled"] = $user.HiddenFromAddressListsEnabled
    $currentuser["DeliverToMailboxAndForward"] = $user.DeliverToMailboxAndForward
    $currentuser["ForwardingAddress"] = $user.ForwardingAddress
    $currentuser["ForwardingSmtpAddress"] = $user.ForwardingSmtpAddress
    $currentuser["MBXSize"] = $MBXStats.TotalItemSize
    $currentuser["MBXSize-GB"] = ([math]::Round(($MBXStats.TotalItemSize.ToString() -replace “(.*\()|,| [a-z]*\)”, “”)/1GB,3))
    $currentuser["TotalDeletedItemSize-GB"] = ([math]::Round(($MBXStats.TotalDeletedItemSize.ToString() -replace “(.*\()|,| [a-z]*\)”, “”)/1GB,3))
    $currentuser["MBXItemCount"] = $MBXStats.ItemCount
        ...
        if ($grantSendOnBehalf = $user.GrantSendOnBehalfTo) {
            ...
            $currentuser["GrantSendOnBehalfTo"] = $null
        }
        ...
        # Archive Mailbox Check
        $currentuser["ArchiveStatus"] = $user.ArchiveStatus
        $currentuser["ArchiveName"] = $user.ArchiveName.ToString()
        if ($user.ArchiveStatus -eq "Active") {
            $ArchiveStats = Get-EXOMailboxStatistics $user.primarysmtpaddress -Archive -ErrorAction silentlycontinue | select TotalItemSize, ItemCount     
        ...
        $hashtable[$user] = $currentuser
}
$sourceMailboxStats | Export-Excel -Path 'C:\Users\amedrano\Arraya Solutions\Spectra - Internal - 1777 - Spectra to OVG- Tenant-to-Tenant Migration\Customer Mailbox List\AllSpectraMailboxes_1-6-2022.xlsx'
...
foreach ($user in $sourceMailboxes) {
    ...
    $currentuser["TotalDeletedItemSize-GB"] = ([math]::Round(($MBXStats.TotalDeletedItemSize.ToString() -replace “(.*\()|,| [a-z]*\)”, “”)/1GB,3))
    $currentuser["MBXItemCount"] = $MBXStats.ItemCount
    $sourceMailboxStats[$user.PrimarySmtpAddress] = $currentuser
}

$elements = "1","2","3","4"
foreach ($element in $elements) {
    $currentElement = @{}
    $currentElement["key1"] = "value1"
    $currentElement["key2"] = "value2"
    # Add other key-value pairs as needed
    
    # Add the current element to the hash table
    $hashTable[$element] = $currentElement
}
