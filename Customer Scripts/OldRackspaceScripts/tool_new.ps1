...
## Last Modified by: Aaron Medrano
## Date Modified 1/2/2020

## Last modification: Add Impersonation Rights and Remove Impersonation Rights in Advanced Menu
...
function Get-AdvancedMenu
{
    ...
    while($AdvSelection -lt 1 -or $AdvSelection -gt 3)
    {
        cls
        Write-Host @"
Advanced Support Menu     (Under Construction)
...
1. Message Tracking
2. Check MSMQ
3. Restart Exchange Tools (LTs Only)
4. IIS Logs (currently unavailable)
5. Set Impersonation Rights (Available for Escalation Techs)
6. Remove-Impersonation Rights (LTs Only)
7. Exit and Restart

"@
        [int]$mainSelection = Read-Host "Select either 1, 2, 3, 4, 5, 6, or 7"
        Switch($mainSelection)
        {
...
"@
        $Script:DomainSelection = Read-Host "Select either 1, 2, 3, 4, 5, 6, 7, 8, 9, or 10"
...
function Get-ExchMessageTracking
{
    ...
    try
    {
        if($sender -ne "" -and $messageID -eq "" -and $recipient -ne "")
        {
            ...
            Get-TransportService | get-messagetrackinglog -resultsize unlimited -recipient $recipient -sender $sender -Start $startDate -End $endDate | select 
Timestamp,EventId,{$_.Sender},RecipientCount,{$_.Recipients},MessageSubject,ReturnPath,RelatedRecipientAddress,{$_.RecipientStatus},InternalMessageId,MessageId,Client-
Ip,ClientHostname,Server-Ip,ServerHostname,SourceContext,ConnectorId,Source,TotalBytes,{$_.Reference},MessageInfo | sort-object Timestamp | export-csv $Script:mydesk
\exports\$Script:domain\$csv -NoTypeInformation
        ...
        elseif($sender -eq "" -and $messageID -eq "" -and $recipient -ne "")
        {
            ...
            Get-TransportService | get-messagetrackinglog -resultsize unlimited -recipient $recipient -Start $startDate -End $endDate -verbose | select 
Timestamp,EventId,Sender,RecipientCount,{$_.Recipients},MessageSubject,ReturnPath,RelatedRecipientAddress,{$_.RecipientStatus},InternalMessageId,MessageId,Client-
Ip,ClientHostname,Server-Ip,ServerHostname,SourceContext,ConnectorId,Source,TotalBytes,{$_.Reference},MessageInfo | sort-object Timestamp | export-csv $Script:mydesk
\exports\$Script:domain\$csv -NoTypeInformation            
        ...
        elseif($recipient -eq "" -and $messageID -eq "" -and $sender -ne "")
        {
            ...
            Get-TransportService | get-messagetrackinglog -resultsize unlimited -sender $sender -Start $startDate -End $endDate | select 
Timestamp,EventId,Sender,RecipientCount,{$_.Recipients},MessageSubject,ReturnPath,RelatedRecipientAddress,{$_.RecipientStatus},InternalMessageId,MessageId,Client-
Ip,ClientHostname,Server-Ip,ServerHostname,SourceContext,ConnectorId,Source,TotalBytes,{$_.Reference},MessageInfo | sort-object Timestamp | export-csv $Script:mydesk
\exports\$Script:domain\$csv -NoTypeInformation
            Write-Host -foregroundcolor cyan "Complete"
        ...
        elseif($messageID -ne "" -and $sender -eq "" -and $recipient -eq "")
        {
            ...
            Get-TransportService | get-messagetrackinglog -resultsize unlimited -messageid $messageID -Start $endDate -End $startDate | select 
Timestamp,EventId,Sender,RecipientCount,{$_.Recipients},MessageSubject,ReturnPath,RelatedRecipientAddress,{$_.RecipientStatus},InternalMessageId,MessageId,Client-
Ip,ClientHostname,Server-Ip,ServerHostname,SourceContext,ConnectorId,Source,TotalBytes,{$_.Reference},MessageInfo | sort-object Timestamp | export-csv $Script:mydesk
\exports\$Script:domain\$csv -NoTypeInformation
            Write-Host -foregroundcolor cyan "Complete"
...
function Get-MSMQCurrentQueue
{
    ...
    foreach ($srv in $srvList)
    {
        ...
        $Result = Invoke-Command -ComputerName IAD3-MSMQ01.mgmt.mlsrvr.com -Credential $Script:MGMTcred -ScriptBlock {Get-MsmqQueue -QueueType Private | ?{$_.path -like 
"*Domain*"}}
        if ($Result -ne $null)
	    {	
		    foreach ($a in $Result)
...
function Get-MailboxInboxRules
{
    ...
    Get-InboxRule -Mailbox $Script:User | select Name,@{Name=’Description’;Expression={[string]::join(";", ($_.Description))}} | Export-Csv $Script:mydesk\exports\
$Script:domain\$csv -NoTypeInformation
}
...
function Get-AllX500Addresses
{
    ...
    get-distributiongroup -OrganizationalUnit $Script:Domain -resultsize unlimited | select pri*, legacy* | export-csv $mydesk\exports\$Script:Domain\$csv1 -
NoTypeInformation
...
function Get-DomainGALObjects
{
    ...
    $objects | select FirstName, LastName, DisplayName, Initials, WindowsEmailAddress, company, Department, Manager, Title, Fax, AssistantName, Notes, Office, @
{Name=’Mobile Phone’;Expression={[string]::join(";", ($_.MobilePhone))}}, @{Name=’Other Fax’;Expression={[string]::join(";", ($_.OtherFax))}}, @{Name=’Home 
Phone’;Expression={[string]::join(";", ($_.HomePhone))}}, @{Name=’Other Home Phone’;Expression={[string]::join(";", ($_.OtherHomePhone))}}, @{Name=’Other 
Telephone’;Expression={[string]::join(";", ($_.OtherTelephone))}}, @{Name=’Pager’;Expression={[string]::join(";", ($_.Pager))}}, @{Name=’Phone’;Expression={[string]::join
(";", ($_.Phone))}}, StreetAddress, StateOrProvince, PostalCode, countryorregion,@{Name=’Post Office Box’;Expression={[string]::join(";", ($_.PostOfficeBox))}} | export-
csv $Script:mydesk\exports\$Script:domain\$csv -NoTypeInformation
}
...
function Get-DomainMailboxSize
{
    ...
    get-Mailbox -OrganizationalUnit $Script:domain -resultsize unlimited | Get-MailboxStatistics | select displayname,itemcount,totalitemsize | export-csv $Script:mydesk
\exports\$Script:domain\$csv -NoTypeInformation
}
...
function Get-CASMailboxFeatures
{
    ...
    get-casmailbox -OrganizationalUnit $Script:domain -resultsize unlimited |  select PrimarySMTPAddress,ActivesyncEnabled,OWAEnabled,POPEnabled,IMAPEnabled,MAPIEnabled  | 
Export-Csv $Script:mydesk\exports\$Script:domain\$csv -NoTypeInformation  
}
...
function Get-DomainPermissions
{
    ...
    ForEach ($Mailbox in $Mailboxes)
    {
        $SendAs = Get-ADPermission $Mailbox.DistinguishedName | ? {$_.ExtendedRights -like "Send-As" -and $_.User -notlike "NT AUTHORITY\SELF" -and $_.User -notlike 
"MEX05\MEX05 Exchange Engineers" -and $_.User -notlike "MEX05\Migrations Team" -and $_.User -notlike "MEX05\Managed Mail L*"}
        ...
        $FullAccess = Get-MailboxPermission $Mailbox.Identity | ? {$_.AccessRights -eq "FullAccess" -and $_.User -notlike "NT AUTHORITY\SELF" -and $_.User -notlike 
"MEX05\MEX05 Exchange Engineers" -and $_.User -notlike "MEX05\Migrations Team" -and $_.User -notlike "MEX05\Managed Mail L*"}
...
### Ignore SSL Certificate funcation
function Ignore-SSLCertificates
{
    ...
    $TASource=@'
        namespace Local.ToolkitExtensions.Net.CertificatePolicy
        {
            ...
            {
                public bool CheckValidationResult(System.Net.ServicePoint sp,System.Security.Cryptography.X509Certificates.X509Certificate cert, System.Net.WebRequest req, 
int problem)
                {
                    return true;
                }
...
function Get-IISLogApp
{
    ...
    {
        Write-Progress -Activity ("Gathering IIS logs for " + $mailbox.PrimarySMTPAddress.ToString()) -Status ("Mailbox " + $i + " of " + ($iisQueryList | Measure-
Object).Count) -PercentComplete (($i / ($iisQueryList | Measure-Object).Count) * 100) -Id 1
...
function Get-IISLogs
{
    ...
    PROCESS
    {
        ...
        foreach ($date in $dateRange)
        {
            ...
            $iisLogQuery  = Get-ElasticData -ElasticUri "https://10.23.232.16" -Index $elasticIndex -SearchKey "cs_username_na" -SearchTerm 
$mailboxObj.PrimarySMTPAddress.ToString()
            $iisLogQuery  += Get-ElasticData -ElasticUri "https://10.23.232.16" -Index $elasticIndex -SearchKey "cs_username_na" -SearchTerm $mailboxObj.SamAccountName
            Write-Progress -Activity ("Searching date " + (Get-Date $date.Date -UFormat "%m/%d/%Y")) -Status ("Query " + $i + " of " + ($dateRange | Measure-Object).Count) 
-PercentComplete (($i / ($dateRange | Measure-Object).Count) * 100) -ParentId 1
            $i++
...
Get-LocalCredentials
$Session = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://ORD2CAS01.mex05.mlsrvr.com/PowerShell/ -Authentication Kerberos -Credential 
$Script:LocalCred
...
Write-Host "Set-Impersonation Rights:
 What is the email address of the service account that will be doing the impersonating? " -ForegroundColor Yellow -NoNewline
...
$filter = "CustomAttribute10 -eq '$strDomainA'"

<#Set up ManagementScope and ManagementRoleAssignment names#>
$scopeName = "AppImpers-$strDomainA"
$roleAssignmentName = "AppImpers-$strDomainA"
...
New-ManagementRoleAssignment –Name $roleAssignmentName  -Role ApplicationImpersonation –User $strSvc_Acct -CustomRecipientWriteScope $scopeName -WarningAction 
SilentlyContinue | Out-Null

<#Output results to screen#>
Write-Host "New Management Scope" -ForegroundColor Cyan
Get-ManagementScope | Where {$_.name -like "*$strDomainA*"} | select Name,RecipientFilter

Write-Host	"New Management Role Assignment" -ForegroundColor Cyan
Get-ManagementRoleAssignment | Where {$_.name -like "*$strDomainA*"} | select Name,CustomRecipientWriteScope,RoleAssignee

Write-Host "`n`n"

Write-Host "*** There is no error handling in this script, so if the above output is blank, try running these commands individually and see if you get errors *** `n" -BackgroundColor Black -ForegroundColor Yellow
...
Write-Host "New-ManagementRoleAssignment –Name $roleAssignmentName  -Role ApplicationImpersonation –User $strSvc_Acct -CustomRecipientWriteScope $scopeName `n" -
BackgroundColor Black -ForegroundColor White

<#Grant Full Access Permissions without Automapping#>
$mbxs_domain = Get-mailbox -organizationalunit $strDomainA -resultsize unlimited
$sam_account = (Get-mailbox $strSvc_Acct).samaccountname

Write-host " 

  Setting Full Access Permissions for for all users on $strDomainA domain" -ForegroundColor White
foreach ($mbx in $mbxs_domain) {
    $addMailboxPerms = add-mailboxpermission -Identity $mbx.identity -user $sam_account -accessrights FullAccess -automapping $false -ErrorVariable 
$mailboxpermission_error -WarningAction SilentlyContinue
    for ($a = 1; $a -le 100; $a++ )
                        {
                          Write-Progress -Activity "Granting '$strSvc_Acct' Full Access to mailbox '$mbx'" -Status "$a% Complete:" -PercentComplete $a;
                        }
...
function Remove-ImpersonationRights
{
<#Check to see if Exchange snapin is loaded.  If not, load it #>
If ( (Get-PSSnapin -Name *exchange* -ErrorAction SilentlyContinue) -eq $null )
    {
...
<#Get variables from user#>
Write-Host " ***This Script does not have a lot of error handling. Double check spelling and ensure the domain exists on this Exchange Environment.
 
What is the email address with impersonation rights to remove? " -ForegroundColor Yellow -NoNewline
$strSvc_Acct = Read-Host
$strdomain = $strSvc_Acct -split '@'
$strDomainA = $strDomain[1]

if (!$strDomainA ) 
{Write-Host "Invalid email address. Please check spelling" -ForegroundColor Red
Read-Host "Press Any Key to Continue"
#return to Impersonation Rights menu
Remove-ImpersonationRights}

    else {
Write-Host ""

<#Remove the ManagementRoleAssignment#>
Write-Host "Checking for RoleAssignment ..." -ForegroundColor Cyan
$ManagementRoleAssignment_Name = Get-ManagementRoleAssignment *$strDomainA* -ErrorAction SilentlyContinue -ErrorVariable $NoRoleAssigned
if ($ManagementRoleAssignment_Name) {$ManagementRoleAssignment_Name | select Name, Role, RoleAssignee
Write-Host "Removing Management Scope" -ForegroundColor Cyan
              $ManagementRoleAssignment_Name | Remove-ManagementRoleAssignment -confirm}
    elseif ($ManagementRoleAssignment_Name.count -gt 1) {
    Write-Host "Unable to Process this request. There are multiple entries with this domain. Please Remove this manually before trying again" -ForegroundColor Red
    }
        else {Write-Host "No Domain Scope found for $strDomainA. Check spelling and try again" -ForegroundColor Red
        }

<#Remove the ManagmentScope#>
Write-Host "Checking for ManagementScope ..." -ForegroundColor Cyan
$ManagementScope_Name = Get-ManagementScope *$strDomainA* -ErrorAction SilentlyContinue -ErrorVariable $NoDomainScope
if ($ManagementScope_Name) {$ManagementScope_Name | select Name, RecipientFilter
Write-Host "Removing Management Role Assignment" -ForegroundColor Cyan
            $ManagementScope_Name | Remove-ManagementScope -confirm
    }
    elseif ($ManagementScope_Name.count -gt 1) {
    Write-Host "Unable to Process this request. There are multiple entries with this domain. Please Remove this manually before trying again" -ForegroundColor Red
    }
        else {Write-Host "No Domain Scope found for $strDomainA. Check spelling and try again" -ForegroundColor Red        
        }

<#Remove Full Access Permissions without Automapping#>
$mbxs_domain = Get-mailbox -organizationalunit $strDomainA -resultsize unlimited
$sam_account = (Get-mailbox $strSvc_Acct).samaccountname

Write-host "
  Removing Full Access Permissions for $strSvc_Acct" -ForegroundColor White
foreach ($mbx in $mbxs_domain) {
    $RemoveMailboxPerms = Remove-mailboxpermission -Identity $mbx -user $sam_account -accessrights FullAccess -Confirm:$false -ErrorVariable $mailboxpermission_error
    for ($a = 1; $a -le 100; $a++ )
                        {
                          Write-Progress -Activity "Removing '$strSvc_Acct' Full Access to mailbox '$mbx'" -Status "$a% Complete:" -PercentComplete $a;
                        }
    if ($mailboxpermission_error.count -eq 1) {Write-Host "There was a problem removing permissions to '$mbx'" -ForegroundColor Red }
        }
     If ($mailboxpermission_error.count -eq 0) {Write-Host "Successfully Removed Full Access Permissions to all Mailboxes" -ForegroundColor Green}

Write-host "
  Full Access Permissions for $strSvc_Acct Removed Successfully" -ForegroundColor Green
