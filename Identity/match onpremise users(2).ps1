...
foreach ($user in ($importcsv | ? {$_.ExistsOnO365 -eq $True})) {
    $upn = $user.UserPrincipalName
    Write-Host "updating immutableid for user $upn"
    Set-MsolUser -userprincipalname $upn -immutableid $user.ImmutableID -whatif
}
...
# match onpremise users against mail attribute
...
foreach ($user in $importcsv) { 
    Write-Host "Checking for $($user.EmailAddress) on premise ..." -fore Cyan -NoNewline
    $UPNLookup = $user.EmailAddress
    $tmp = "" | select DisplayName, Username, EmailType, EmailAddress, ExistsOnPrem, ADUPN, DistinguishedName
    $tmp.Username = $user.Username
    $tmp.EmailType = $user.MailType
    $tmp.EmailAddress = $user.EmailAddress    
    
    if ($ADUser = Get-AdUser -filter {mail -eq $UPNLookup}) {
    ...
    {
        $notfoundusers += $user.EmailAddress
        Write-Host "not found" -ForegroundColor red
        $tmp.ExistsOnPrem = $False
    }
...
# match onpremise groups against primarysmtp ROR
...
foreach ($user in $mailenabledgroups ) { 
    Write-Host "Checking for $($user.PrimarySmtpAddress) on premise ..." -fore Cyan -NoNewline
    $DLLookup = $user.PrimarySmtpAddress
    $tmp = "" | select Identity, PrimarySmtpAddress, ExistsOnPrem, IsDirSynced, EXODisplayName
    $tmp.Identity = $user.Identity
    $tmp.PrimarySmtpAddress = $user.PrimarySmtpAddress
    $tmp.IsDirSynced = $user.IsDirSynced
    
    if ($DLGroup = Get-Recipient $DLLookup -ea silentlycontinue)
    {
        
        $foundGroups += $user.PrimarySmtpAddress
        Write-Host "found" -ForegroundColor Green
        $tmp.EXODisplayName = $DLGroup.DisplayName
        $tmp.ExistsOnPrem = $true
    }

    else
    {
        $notFoundGroups += $user.PrimarySmtpAddress
        Write-Host "not found" -ForegroundColor red
        $tmp.ExistsOnPrem = $False
    }
