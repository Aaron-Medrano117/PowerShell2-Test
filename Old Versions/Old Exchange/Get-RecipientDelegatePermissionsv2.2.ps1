...
param (
    ...
    [Parameter(Mandatory=$false,HelpMessage="Run against Office365 Exchange Online?")] [switch]$AllPerms,
...
$TotalCount = ($allRecipients).count

#Build Array
$CollectPermissionsList = @()
$failedUsers = @()
foreach ($obj in $allRecipients) {
    Write-ProgressHelper -ProgressCounter ($progresscounter++) -Activity "Gathering Details for $($obj.DisplayName)" -ID 1 -TotalCount $TotalCount -StartTime $global:Initialstart
...
$CollectPermissionsList | Export-Excel "$HOME\Desktop\All-DelegatePermissions-012023.xlsx"
