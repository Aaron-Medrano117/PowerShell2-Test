...
foreach ($obj in $allRecipients) {
    ...
    if ($SendAs) {
        ...
        if ($OnPremises) {
            [array]$SendAsPerms = Get-User $obj.Identity.ToString() -EA SilentlyContinue | Get-ADPermission | Where {$_.ExtendedRights -like "Send-As" -and $_.User -notlike "NT AUTHORITY*" -and $_.User -notlike "*S-1-*"}         
        }
        elseif ($Office365) {
            [array]$SendAsPerms = Get-ExoRecipientPermission $obj.Identity.ToString() -EA SilentlyContinue | Where {$_.Trustee.ToString() -ne "NT Authority\Self" -and $_.Trustee.ToString() -notlike "*S-1-*"}
        }
        ...
        if ($SendAsPerms) {
            ...
            foreach ($perm in $SendAsPerms) {
                ...
                if ($AllGroupMembers) {
                    if ($recipientCheck.RecipientTypeDetails -like "*Group*") {
                        ...
                        if ($GroupMembers = Get-DistributionGroupMember $permUser) {
                            ...
                            $currentPerm | add-member -type noteproperty -name "Identity" -Value $obj.Identity.ToString()
                ...
                #All Else, Output Perm users details
                    ...
                    $currentPerm | add-member -type noteproperty -name "Identity" -Value $obj.Identity.ToString()
                    $currentPerm | add-member -type noteproperty -name "MailObject" -Value $primarySMTPAddress
                    $currentPerm | add-member -type noteproperty -name "RecipientTypeDetails" -Value $obj.RecipientTypeDetails
                    $currentPerm | add-member -type noteproperty -name "PermissionType" -Value "SendAs"
    ...
    if ($CalendarPermissions) {
        if ($recipientTypeDetails -like "*Mailbox") {
            ...
            if ($OnPremises) {[array]$calendars = Get-MailboxFolderStatistics $obj.Identity.ToString() | Where {$_.FolderPath -eq "/Calendar" -or $_.FolderPath -like "/Calendar/*"}}
            elseif ($Office365) {[array]$calendars = Get-EXOMailboxFolderStatistics $obj.Identity.ToString() | Where {$_.FolderPath -eq "/Calendar" -or $_.FolderPath -like "/Calendar/*"}}
