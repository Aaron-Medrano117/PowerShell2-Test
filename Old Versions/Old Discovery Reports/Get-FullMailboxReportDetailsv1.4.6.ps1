...
function Get-AllOneDriveDetails {
    ...
    foreach ($site in $OneDriveSite) {
        ...
        if ($user = Get-MgUserLicenseDetail -UserId $site.Owner -ErrorAction SilentlyContinue -ErrorVariable +allErrors) {
            $isActive = $true
        }
        else {
            $isActive = $false
...
function Combine-AllMailboxStats {
    ...
    #Progress Bar Parameters Reset
    $tenantStatsHash["AllMailboxDetails"] = @{}
    ...
    foreach ($mailbox in $tenantStatsHash["AllMailboxes"].values) {
        ...
        $onmicrosoftAlias = ($EmailAddresses | Where-Object { $_ -like "*.onmicrosoft.com" } | Select-Object -First 1).Replace("smtp:","")
        ...
        $tenantStatsHash["AllMailboxDetails"] = $allMailboxStats
